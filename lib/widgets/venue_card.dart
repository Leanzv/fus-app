...
class VenueCard extends StatelessWidget {
  ...
  const VenueCard({super.key, required this.venue, required this.onTap,
    this.showOwnerActions = false, this.onEdit, this.onDelete});
            ...
            Stack(children: [
              ...
              if (venue.averageRating != null)
                Positioned(top: 10, right: 10, child: Container(
                  ...
                  child: Row(mainAxisSize: MainAxisSize.min, children: [
                    const Icon(Icons.star, color: AppTheme.starColor, size: 13),
              ...
              if (showOwnerActions)
                Positioned(bottom: 8, right: 8, child: Row(children: [
                  if (onEdit != null) _ActionBtn(icon: Icons.edit_outlined,
                      onTap: onEdit!, color: AppTheme.secondaryColor),
                  const SizedBox(width: 6),
                  if (onDelete != null) _ActionBtn(icon: Icons.delete_outline,
                      onTap: onDelete!, color: AppTheme.errorColor),
                ])),
            ]),
            Padding(padding: const EdgeInsets.all(14), child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(venue.name, style: const TextStyle(fontSize: 16,
                    fontWeight: FontWeight.w700, color: AppTheme.textPrimary),
                    maxLines: 1, overflow: TextOverflow.ellipsis),
                const SizedBox(height: 6),
                Row(children: [
                  const Icon(Icons.star, color: AppTheme.starColor, size: 14),
                  Text(' ${venue.averageRating?.toStringAsFixed(1) ?? "Belum ada rating"}',
                      style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w600)),
                  Text(' · ${venue.reviewCount ?? 0} ulasan',
                      style: const TextStyle(fontSize: 12, color: AppTheme.textSecondary)),
                  const Spacer(),
                  if (venue.distanceKm != null) Row(children: [
                    const Icon(Icons.near_me, color: AppTheme.secondaryColor, size: 14),
